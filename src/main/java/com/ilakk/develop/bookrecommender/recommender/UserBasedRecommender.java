...
public class UserBasedRecommender implements IRecommendations {

	private DataModel model;

	private RecommenderBuilder recommenderBuilder;

	public UserBasedRecommender(DataModel model, RecommenderBuilder recommenderBuilder) {
		this.model = model;
	...
	public String recommend(int userid) {
		...
		try {
			recommendations = recommenderBuilder.buildRecommender(model).recommend(userid, 20);
